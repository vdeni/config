...
then
  ...
  then
    while read -r line; do
      echo "$line" \?\?
    done <<< $(find /home/denis -iregex "${QUERY#\?}" 2>&1 | grep -v 'Permission denied\|Input/output error')
  else
    find /home/denis -iregex "${QUERY#!}" 2>&1 | grep -v 'Permission denied\|Input/output error'
  fi
else
  echo "!!-- Type your search query to find files"
